...
export default {
  ...
  methods: {
    addProductToCart() {
      // console.log("adding product to cart: " + JSON.stringify(this.product.variant_groups[0].options[0].id))
      const product = {
        productId: this.product.id,
        variant: {}
        // {
        //   [this.product.variant_groups[0].id]: this.product.variant_groups[0].options[0].id
        // }
      }
